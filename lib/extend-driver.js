 ...
 * @param {Object} webdriver - The `selenium-webdriver` module you're using, i.e. `require('selenium-webdriver')`. This library
 ...
 */
function extendDriver(driver, webdriver, options) {
  _.defaults(options, {
   ...
   */
  driver.helpers.Key = webdriver.Key;
   ...
   * @memberof ExtendedDriver
   */
  driver.helpers.until = webdriver.until;
   ...
   */
  driver.helpers.selectElement = selector => {
   ...
   * @return {Promise<WebElement>}
   */
  driver.helpers.waitForElementNotToBeVisible = (selector, timeout) => {
    timeout = timeout || 10e3;
   ...
   * @return {Promise<Thenable>}
   */
  driver.helpers.waitForUrl = (pattern, timeout) => {
    timeout = timeout || 10e3;
   ...
   * @return {Promise<Thenable>}
   */
  driver.helpers.waitForElementNotToHaveClass = (selector, className, timeout) => {
    timeout = timeout || 10e3;
    return driver.helpers.waitForLoaders(timeout).then(() => {
      return driver.wait(
        webdriver.until.elementLocated(
